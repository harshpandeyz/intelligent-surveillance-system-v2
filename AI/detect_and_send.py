...
BACKEND_URL = os.getenv("BACKEND_URL", "http://127.0.0.1:8153")
...
_MODEL_FILE = os.getenv("MODEL_PATH", str(_root_dir / "AI" / "yolov8n.pt"))
...
def encrypt_file(in_path, out_path, key=None):
    aesgcm = AESGCM(key or get_aes_key())
    nonce = os.urandom(12)
...
_detect_send_token = None   # module-level cache


def _fetch_token_for_detect():
    global _detect_send_token
    try:
        res = requests.post(
            f"{BACKEND_URL}/login",
            data={
                "username": os.getenv("AI_ADMIN_USER", "admin"),
                "password": os.getenv("AI_ADMIN_PASS", "admin123"),
            },
            headers={"Content-Type": "application/x-www-form-urlencoded"},
            timeout=10,
        )
        res.raise_for_status()
        _detect_send_token = res.json().get("access_token")
    except Exception as e:
        print("[ERROR] detect_and_send: token fetch failed:", e)
        _detect_send_token = None
...
def post_event(payload):
    ...
    if not is_valid_hex(hash_hex):
        print(f"[WARN] Invalid hex hash, regenerating: {payload.get('enc_path')}")
        hash_hex = compute_sha256_file(payload.get("enc_path"))
        payload["hash"] = hash_hex

    if not _detect_send_token:
        _fetch_token_for_detect()

    headers = {"Authorization": f"Bearer {_detect_send_token}"} if _detect_send_token else {}
    try:
        resp = requests.post(
            f"{BACKEND_URL}/event", json=payload, headers=headers, timeout=10
        )
        if resp.status_code == 401:
            # Token expired — refresh once and retry
            _fetch_token_for_detect()
            headers = (
                {"Authorization": f"Bearer {_detect_send_token}"}
                if _detect_send_token
                else {}
            )
            resp = requests.post(
                f"{BACKEND_URL}/event", json=payload, headers=headers, timeout=10
...
def main(camera_source=0, device='cpu'):
    ...
    if not cap.isOpened():
        print(f"[ERROR] Cannot open camera source: {camera_source}. "
              "Set CAMERA_SOURCE in .env to a valid device or RTSP URL.")
        return
