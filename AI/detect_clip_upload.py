...
ACCESS_TOKEN = None  # fetched lazily on first use

# Load models
MODEL_FILE = os.getenv("MODEL_PATH", str(root_path / "AI" / "yolov8n.pt"))
...
def post_event(payload):
    ...
    try:
        ...
        enc = payload.get("enc_path")
        if enc and os.path.exists(enc):
            hash_hex = compute_sha256_file(enc)
            payload["hash"] = hash_hex
        else:
            print("[WARN] Cannot fix invalid hash.")
